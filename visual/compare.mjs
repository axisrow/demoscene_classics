...
// Foreground ratio: the fraction of pixels that visibly deviate from the image's
// dominant (background) colour. A sparse effect on a dark canvas (starfield's
// stars) has a small but nonzero foreground ratio; a completely blank render
// (the effect drew nothing) has a foreground ratio of ~0 regardless of the
// intended content. Used as a semantic blank guard so a regression that empties
// an effect cannot hide inside a permissive diff-pixel-ratio tolerance.
//
...
export function foregroundRatio(decoded, channelTolerance = 16) {
  const { rgba } = decoded;
  const n = rgba.length / 4;
  if (n === 0) return 0;
  // Dominant colour via a coarse 4-bit/channel histogram (12-bit buckets). This
  // is robust to per-pixel AA noise and cheap; exact dominant-colour detection
  // is not needed for a blank/empty guard.
...
// at least this fraction of foreground (non-background) pixels, computed
// independently from each image's own dominant colour. A capture that drops to
// ~0 foreground (the effect rendered nothing) fails even when its diff-pixel
// ratio is small — which matters for sparse vector effects (starfield) whose
// intended content is a small fraction of the frame. Defaults to 0 (off).
//
