...
// per (surface × device) combination. The texture identity (checker/radial
// pattern geometry, rotation and zoom motion) is owned by config.js; profiles
// only tune execution budgets. Fullscreen slots keep the classic composition
// (render.resolution stays at the 0.5 default); preview slots lower the
// sampling cost to the value the gallery previously used, so the card shows the
// same rotating checker/radial texture at a coarser buffer.
//
...
// 1 and viewport auto-pause is enabled everywhere until the rotozoom tuning
// issue (#12) revisits them.
...
const PREVIEW_RENDER = { render: { resolution: 0.25 } };
