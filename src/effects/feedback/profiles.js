...
// Responsive profiles for feedback. Four explicit, effect-owned slots — one
// per (surface × device) combination. The feedback-loop identity (sides,
// passes, decay, scale, rotation) is owned by config.js; profiles only tune
// execution budgets. Fullscreen slots keep the classic composition
// (render.resolution stays at the 1.0 vector default); preview slots lower the
// sampling cost to the value the gallery previously used, so the card shows the
// same recursive trail pattern at a coarser buffer.
//
...
// 1 and viewport auto-pause is enabled everywhere until the feedback tuning
// issue (#13) revisits them.
